...
export const AREAS: AreaConfig[] = [
  { key: "나", label: "나 (Me)", emoji: "🌱", color: "#4ade80", dbEnvKey: "NOTION_나_DB_ID" },
  { key: "일", label: "일 (Work)", emoji: "💼", color: "#60a5fa", dbEnvKey: "NOTION_일_DB_ID" },
  { key: "관계", label: "관계 (People)", emoji: "🤝", color: "#f472b6", dbEnvKey: "NOTION_관계_DB_ID" },
  { key: "배움", label: "배움 (Learn)", emoji: "📚", color: "#a78bfa", dbEnvKey: "NOTION_배움_DB_ID" },
  { key: "건강", label: "건강 (Health)", emoji: "🏃", color: "#fb923c", dbEnvKey: "NOTION_건강_DB_ID" },
  { key: "가족", label: "가족 (Family)", emoji: "🏡", color: "#f9a8d4", dbEnvKey: "NOTION_가족_DB_ID" },
  { key: "경제적 자유", label: "경제적 자유 (Finance)", emoji: "💰", color: "#fbbf24", dbEnvKey: "NOTION_경제적자유_DB_ID" },
];
...
export async function fetchAchievements(limit = 20): Promise<Achievement[]> {
  const notion = getClient();
  const dbId = process.env.NOTION_이룸_DB_ID;
  if (!dbId) return [];
